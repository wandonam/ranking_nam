...
def main() -> None:
    ...
    if not args.skip_aggregate:
        logger.info("[1/4] 월간 데이터 집계 시작")
        try:
            aggregate.run(year, month)
            logger.info("[1/4] 집계 완료")
        except Exception as e:
            logger.error(f"[1/4] 집계 실패: {e}")
            sys.exit(1)
    else:
        logger.info("[1/4] 집계 건너뜀 (--skip-aggregate)")

    # ── 2단계: 히어로 선발
    if not args.skip_hero:
        logger.info("[2/4] 히어로 제품 선발 시작")
        try:
            heroes = hero.run(year, month)
            logger.info(f"[2/4] 히어로 선발 완료 - {len(heroes)}개 채널")
        except Exception as e:
            logger.error(f"[2/4] 히어로 선발 실패: {e}")
            sys.exit(1)
    else:
        logger.info("[2/4] 히어로 선발 건너뜀 (--skip-hero)")

    # ── 3단계: 카드 HTML 생성
    if not args.skip_cards:
        logger.info("[3/4] 히어로 카드 HTML 생성 시작")
        try:
        ...
        logger.info(f"[3/4] 카드 HTML 생성 완료")
    else:
        logger.info("[3/4] 카드 HTML 생성 건너뜀 (--skip-cards)")

    # ── 4단계: PNG 변환
    if not args.skip_png:
        logger.info("[4/4] PNG 변환 시작")
        try:
            r = html2png.run_folder(str(monthly_folder))
            total_success += r["success"]
            total_fail    += r["fail"]
            logger.info(f"[4/4] PNG 변환 완료")
        except Exception as e:
            logger.error(f"[4/4] PNG 변환 실패: {e}")
            total_fail += 1
    else:
        logger.info("[4/4] PNG 변환 건너뜀 (--skip-png)")
